...
static void aes128_cfb8_encrypt(
  const uint8_t *in, uint8_t *out,
  size_t len, const uint32_t rk32[44], uint8_t iv[16]
) {
  uint8_t rk_bytes[176];
  for (int i = 0; i < 44; i++) {
    rk_bytes[i*4+0] = (uint8_t)(rk32[i] >> 24);
    rk_bytes[i*4+1] = (uint8_t)(rk32[i] >> 16);
    rk_bytes[i*4+2] = (uint8_t)(rk32[i] >> 8);
    rk_bytes[i*4+3] = (uint8_t)(rk32[i]);
  }
...
static void aes128_cfb8_encrypt(
  ...
  __m128i rk[11];
  aes128_key_expand_ni(key_bytes, rk);

  uint8_t sr[16];
  memcpy(sr, iv, 16);

for (size_t i = 0; i < len; i++) {
  __m128i state = _mm_loadu_si128((const __m128i *)sr);
...
int main(int argc, char **argv) {
  const char *path = (argc > 1) ? argv[1] : "the-all-seeing.jpg";
  ...
  if (!file_buf) { perror("malloc"); return 1; }
  size_t rd = 0;
  while (rd < file_len) {
    ssize_t n = read(fd, file_buf + rd, file_len - rd);
  ...
  uint8_t *scratch = malloc(file_len);
  if (!scratch) { perror("malloc"); return 1; }

  for (int i = 0; i < WARMUP; i++) {
    uint8_t tmp_iv[16];
    memcpy(tmp_iv, iv, 16);
    aes128_cfb8_encrypt(file_buf, scratch, file_len, rk, tmp_iv);
  }

  double *results = malloc(sizeof(double) * (size_t)N);
  double sum = 0;
  for (int i = 0; i < N; i++) {
    uint8_t tmp_iv[16];
    memcpy(tmp_iv, iv, 16);
  ...
  double stddev = __builtin_sqrt(var / N);

  qsort(results, (size_t)N, sizeof(double), cmp_double);
  double p50 = results[N / 2];
  double p99 = results[(int)(N * 0.99)];
